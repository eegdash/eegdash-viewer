...
| **MEF3**     | `.mefd/` (directory) | **Metadata-only** | (iEEG spec, same as NWB)                                                | `tests/fixtures/ieeg/mef-tiny.mefd/`                                                                                                               | Parses `.tmet` segment metadata; `readWindow()` throws because RED decompression of `.tdat` is deferred. Viewer must show "Decompression not yet supported" cleanly. |
...
| MEF3                              | `metadata-only` — open succeeds, render throws clean error. |
